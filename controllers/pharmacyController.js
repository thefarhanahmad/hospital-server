...
exports.getInventory = catchAsync(async (req, res) => {
  const inventory = await PharmacyInventory.find({ pharmacy: req.user.pharmacy })
    .populate('medicine')
    .sort('medicine.name');
...
exports.updateInventory = catchAsync(async (req, res, next) => {
  const { medicineId, batchNumber, quantity, expiryDate, purchasePrice, sellingPrice } = req.body;
...
exports.createBill = catchAsync(async (req, res, next) => {
  ...
  for (const item of items) {
    ...
    if (inventory.quantity < item.quantity) {
      return next(new AppError(`Insufficient stock for item: ${inventory.medicine.name}`, 400));
    }
  ...
  const totalDiscount = validatedItems.reduce((acc, item) => acc + item.discount, 0);
...
exports.getBills = catchAsync(async (req, res) => {
  const bills = await PharmacyBill.find({ pharmacy: req.user.pharmacy })
